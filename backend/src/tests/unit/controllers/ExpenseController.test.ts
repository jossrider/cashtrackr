...
jest.mock('../../../models/Expense', () => ({ create: jest.fn() }))
...
describe('ExpenseController.create', () => {
  it('should create a new expense', async () => {
    ...
    const req = createRequest({
      ...
      body: { name: 'Test Expense', amount: 5000 },
      budget: { id: 1 },
    })
    ...
    expect(data).toEqual('Gasto registrado correctamente!!')
...
describe('ExpenseController.updateById', () => {
  it('should update expense and return a success message', async () => {
    const expenseMock = { ...expenses[0], update: jest.fn().mockResolvedValue(true) }
    const req = createRequest({
      method: 'PUT',
      url: '/api/budgets/:budgetId/expenses/:expenseId',
      expense: expenseMock,
...
describe('ExpenseController.deleteById', () => {
  it('should delete expense and return a success message', async () => {
    const expenseMock = { ...expenses[0], destroy: jest.fn().mockResolvedValue(true) }
    const req = createRequest({
      method: 'DELETE',
      url: '/api/budgets/:budgetId/expenses/:expenseId',
      expense: expenseMock,
