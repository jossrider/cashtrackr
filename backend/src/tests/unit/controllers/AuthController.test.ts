...
describe('AuthController.createAccount', () => {
  ...
  it('should return a 409 status and a error message if the email already registered', async () => {
...
describe('AuthController.login', () => {
  ...
  it('should return a 403 if the account has not confirmed', async () => {
    ;(User.findOne as jest.Mock).mockResolvedValue({ id: 1, email: 'test@test.com', password: 'password', confirmed: false })
    const req = createRequest({
      method: 'POST',
      url: '/api/auth/login',
      body: { email: 'test@test.com', password: 'testpassword' },
  ...
  it('should return a 401 if the password is incorrect', async () => {
    const userMock = { id: 1, email: 'test@test.com', password: 'testpassword', confirmed: true }
