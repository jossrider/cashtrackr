...
jest.mock('../../../models/Budget', () => ({ findAll: jest.fn(), create: jest.fn(), findByPk: jest.fn() }))
...
describe('BudgetController.create', () => {
  it('Should create a new budget and response with statusCode 201', async () => {
    ...
    const req = createRequest({
      ...
      url: '/api/budgets',
      user: { id: 10 },
      body: { name: 'Preuspuesto Prueba', amount: 1000 },
    })
    const res = createResponse()
    await BudgetController.create(req, res)
    const data = res._getJSONData()
    expect(res.statusCode).toBe(201)
    expect(data).toBe('Presupuesto creado!!')
...
describe('BudgetController.deleteById', () => {
  it('should delete the budget and return a success message', async () => {
    ...
    const data = res._getJSONData()
    expect(res.statusCode).toBe(200)
    expect(data).toBe('Presupuesto Borrado!!')
