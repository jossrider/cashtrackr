...
router.post(
  '/login',
  body('email').isEmail().withMessage('Email no válido!!'),
  body('password').notEmpty().withMessage('Ela password es obligatprio!!'),
  handleInputErrors,
  AuthController.login
)

router.post('/forgot-password', body('email').isEmail().withMessage('Email no válido!!'), handleInputErrors, AuthController.forgotPassword)
...
router.post(
  ...
  body('new_password').isLength({ min: 8 }).withMessage('El password nuevo es muy corto, mínimo 8 caracteres!!'),
  handleInputErrors,
