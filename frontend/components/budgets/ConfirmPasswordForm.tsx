...
import { deleteBudget } from '@/app/actions/delete-budget-action'
...
export default function ConfirmPasswordForm() {
  ...
  const [state, dispatch] = useActionState(deleteBudgetWithPassword, { errors: [], success: '' })
  useEffect(() => {
    if (state.success) {
    ...
    <>
      ...
      <form className=' mt-14 space-y-5' noValidate action={dispatch}>
        <div className='flex flex-col gap-5'>
          ...
          <input id='password' type='password' placeholder='Password' className='w-full border border-gray-300 p-3 rounded-lg' name='password' />
        </div>
